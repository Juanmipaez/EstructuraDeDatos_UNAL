...
class AVLTree(object):
    ...
    def SubArbolCompleto(self):
        self.maxAlturaCompleta = 0
        self._SubArbolCompletoRecursivo(self.root)
        return self.maxAlturaCompleta
    
    def _SubArbolCompletoRecursivo(self, root):
        if root is None:
            return True, 0
        
        completo_izq, alt_izq = self._SubArbolCompletoRecursivo(root.left)
        completo_der, alt_der = self._SubArbolCompletoRecursivo(root.right)

        if (completo_izq and completo_der and alt_izq == alt_der):
            altura_actual = alt_izq + 1
            self.maxAlturaCompleta = max(self.maxAlturaCompleta, altura_actual)
            return True, altura_actual
        else:
            return False, max(alt_izq, alt_der) + 1
...
Alturas = []
for arbol_subn in arboles:
    arbol = AVLTree()
    for _ in arbol_subn:
        arbol.insert(_)
    Alturas.append(arbol.SubArbolCompleto())
