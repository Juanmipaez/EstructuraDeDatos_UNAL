...
for caso in range(casos):
    valores = list(map(int, input().split()))[:-1]
    arbol = AVLTree()
    for valor in valores:
        arbol.insert(valor)
    
    while arbol._getNodeHeight(arbol.root) > 1:
        maximo = int(arbol.popMax())
        minimo = int(arbol.popMin())
        resta = maximo - minimo
        if arbol.search(resta) == False:
            arbol.insert(resta)
    Masas.append(arbol.root.key)
